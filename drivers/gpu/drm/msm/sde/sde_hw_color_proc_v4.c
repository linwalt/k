...
static unsigned short kcal_red = 180;
static unsigned short kcal_green = 180;
static unsigned short kcal_blue = 180;
static unsigned short kcal_hue = 0;
static unsigned short kcal_sat = 204;
static unsigned short kcal_val = 204;
static unsigned short kcal_cont = 204;
...
void sde_setup_dspp_pccv4(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	for (i = 0; i < PCC_NUM_PLANES; i++) {
		...
		SDE_REG_WRITE(&ctx->hw, base + PCC_R_OFF,
			i == 0 ? (coeffs->r * kcal_red) / 180 : coeffs->r);
		// GREEN
		SDE_REG_WRITE(&ctx->hw, base + PCC_G_OFF,
			i == 1 ? (coeffs->g * kcal_green) / 180 : coeffs->g);
		// BLUE
		SDE_REG_WRITE(&ctx->hw, base + PCC_B_OFF,
			i == 2 ? (coeffs->b * kcal_blue) / 180 : coeffs->b);
